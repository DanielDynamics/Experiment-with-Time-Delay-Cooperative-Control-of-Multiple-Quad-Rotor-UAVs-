 ...
 * Modified from original Servo Class created by vincennt JAUNET (vincent.jaunet@hotmail.fr)
...
void Servo::init()
{
  ...
  //Let ESC get ready to set throttle
  for (int i=0;i<4;i++){
    servoval[i]= Control.thr;
  };
  setServo();
  sleep(3);
...
void Servo::update_roll(float throttle, float PIDoutput_roll)
{
   servoval[1] =(int)(throttle - PIDoutput_roll );
   servoval[3] =(int)(throttle + PIDoutput_roll );
   setServo();
}
